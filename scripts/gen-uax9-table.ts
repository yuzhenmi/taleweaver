...
const dataDir = join(here, "../packages/core/src/layout/uax9/data");
...
// committed source (see bidi-class-table.test.ts).
const outFile = process.env.UAX9_OUT ??
  join(here, "../packages/core/src/layout/uax9/bidi-class-table.ts");
...
console.log(
  `Wrote ${ranges.length} bidi-class ranges (${classNames.length} classes) to ${outFile}`,
);
