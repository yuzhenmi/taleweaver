...
| E-E: #172 LineBox-canonical refactor | ✅ done | spec `cc6e0d9`, E-E.1 `de3f307`, E-E.2 `78a1eed`, E-E.3 `2681145`, E-E.4 `f5d1a9a`, E-E.5 `ba0252d`, E-E.6 `3776452`, E-E.7 `5b25dda` | 7 sub-tasks shipped + spec. LineBox now carries `ownerBlockId / inlineOffsetStart / inlineOffsetEnd / isBlockBoundaryLine` stamped by the IFC at line-emit time. All 5 geometry consumers (hit-test, cursor-position, selection-geometry, line-navigation, paint-indirect) walk LineBoxes directly via `collectLineBoxes`. Synthetic-strut machinery removed; `editor/layout-utils.ts` deleted (cursor → editor layering inversion closed). Embed items render as `display: inline-block, inlineSize: 0` so the IFC offset accumulator matches state-model offsets exactly. Closes #172, E-A9, E-A10, E-A11, D3, #202, E-A15 (resolved structurally). E-A5 partially addressed (constant-factor improvement; per-layout caching is a future task). 14 reviewer passes across the 8 commits. Build clean; 1182 tests pass (was 1161 before E-E series). |
| E-F: Smaller items (E-A12 + D4) | ✅ done | `356eb32` | Two items shipped: (E-A12) `selectionAllHaveAttr` now starts at the first overlapping item via `findItemAtOffset`, exits on `cursor >= rangeEnd`, and early-returns false on the first non-attr text item. Prior implementation walked every item in every spanned block. (D4) `collectAllTextBoxes` LineBox-margin TODO replaced with a stable comment describing why margins are zeroed at the LineBox boundary. Inadvertently-resolved siblings tracked separately: D2 fixed by E-B (cursor/selection.ts header rewrite); E-A17 fixed by E-D (hit-test rewrite eliminated the `as BlockId` cast). Deferred siblings carved out: D1 (rebuildTrees unused param — coupled to incremental wiring, pending), D3 (collectBlockBoundaryLines — dissolves with E-E), E-A15 (moveToLineBoundary cross-block back-up defense gap — task #212), E-A16 (PASTE leading-whitespace policy — task #213). One reviewer pass. |
| E-G: #175 Ctrl+ArrowLeft keybinding | ✅ no change — misdiagnosis | n/a | Investigated `dom/key-handler.ts` against cross-platform conventions (Google Docs, MS Word, Pages, browser textarea/contenteditable). Current keymap correctly routes Linux/Windows `Ctrl+ArrowLeft/Right` → `MOVE_WORD` (standard convention; Home/End is the line-boundary shortcut on Win/Linux), and Mac `Cmd+ArrowLeft/Right` → `MOVE_LINE_BOUNDARY` (standard Mac convention; Option+Arrow is the word shortcut). The task description "falls to MOVE_WORD instead of line-boundary" is reversed — MOVE_WORD IS the expected behavior on Linux/Windows. Existing tests (`packages/dom/src/key-handler.test.ts:60`) lock this in. Closes #175 as no-fix-needed. |
