 ...
 *   2. If the text is non-empty, insert it at the (normalized) anchor
 *      position via insertText.
 *
 ...
 * operations' dirtyIds.
 *
 * Error contract delegates to deleteRange (existence, leaf, cross-parent,
 * cross-context, offset bounds) and insertText (offset bounds for the
 * insert-only case). After deleteRange succeeds, the cursor position is
 * always valid in the post-delete anchor block, so insertText's defensive
 * existence/offset throws are unreachable from this composition.
 *
 * Behavior:
...
export function replaceRange(
  ...
  // Pure no-op: empty span + empty text. Skip both delegation calls.
  if (isCollapsed && text === "") {
    return { state, dirtyIds: new Set<BlockId>() };
  }

  // Normalize once. The normalized anchor position is the cursor
  // position after deletion. normalizeSpan throws via comparePositions
  // on cross-context (no common ancestor); for collapsed same-block
  // spans, it short-circuits without throwing.
  const normalized = normalizeSpan(state, span);

  let workingState = state;
  const combinedDirty = new Set<BlockId>();

  if (!isCollapsed) {
    // Pass the normalized span; deleteRange runs normalizeSpan again
    // internally (idempotent for an already-normalized span). deleteRange
    // owns existence + leaf + cross-parent validation and produces the
    // correct prefixed error messages.
    const deleteResult = deleteRange(workingState, normalized);
    workingState = deleteResult.state;
    for (const id of deleteResult.dirtyIds) combinedDirty.add(id);
  }

  if (text !== "") {
    // The cursor lands at the normalized anchor's blockId + offset.
    // For collapsed spans, this is just span.anchor.
    // For non-collapsed spans (post-delete), this is the seam in the
    // surviving anchor block.
    const cursorPos = createPosition(normalized.anchor.blockId, normalized.anchor.offset);
    const insertResult = insertText(workingState, cursorPos, text, attrs);
    workingState = insertResult.state;
    for (const id of insertResult.dirtyIds) combinedDirty.add(id);
  }

  return { state: workingState, dirtyIds: combinedDirty };
}
```
