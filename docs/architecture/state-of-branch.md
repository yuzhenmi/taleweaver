...
- **Manual hyphenation** (`hyphens: manual`, the cascade default — authored
  ...
  `1.6-text.md` Hyphenation). **Dictionary `auto` hyphenation** is `[implemented]`:
  the `language` + `hyphenateLimitChars` cascade properties, the injected
  `Hyphenator` capability (`EditorConfig.hyphenator`, interface + mock in
  `layout/hyphenator.ts`, threaded to every tokenization site), the AUTO producer
  arm in the IFC (gated on `cs.hyphens==="auto"` + `cs.language` + an injected
  hyphenator, applying `cs.hyphenateLimitChars`), the measure-pass `_metaCache`
  hyphenator guard, AND the concrete Liang hyphenator + `en-us` patterns now
  shipping in `packages/hyphenation` (`createLiangHyphenator()` — a pure
  Knuth-Liang trie over the public-domain TeX `hyphen.tex` patterns; the example
  app wires it). Adding a language is data (register another pattern set), no core
  change. When NO hyphenator is configured, `auto` falls back to `manual` (honor
  soft hyphens, no automatic breaks) — the correct CSS-UA behavior when no
  hyphenation resource exists, not a degraded build.
...
- **Cursor placement within a word broken across lines** — `[implemented]` for BOTH
  ...
  shrink-to-fit / inline-block box can narrow to one cluster. Dictionary `auto`
  hyphenation is complete: the concrete Liang hyphenator + `en-us` patterns ship in
  `packages/hyphenation` (see Manual/Dictionary hyphenation above). `word-break:
  break-all/keep-all` is deliberately OUT of scope (no serious word processor
  exposes it). (The
...
- **Layout — collect + resolve** (`1.5-pagination.md`): the layout-field `FieldSpec` is
