...
describe("undo history grouping", () => {
  it("merges rapid INSERT_TEXT actions into a single undo entry", () => {
    ...
    expect(state.historyLegacy.undoStack.length).toBe(1);
  ...
  it("does not merge across SPLIT_NODE", () => {
    ...
    expect(state.historyLegacy.undoStack.length).toBe(3);
  });
  ...
  it("merges rapid DELETE_BACKWARD actions into a single undo entry", () => {
    ...
    state = typeChars(state, "hello");
    const undoCountAfterType = state.historyLegacy.undoStack.length;
    ...
    expect(state.historyLegacy.undoStack.length).toBe(undoCountAfterType + 1);
