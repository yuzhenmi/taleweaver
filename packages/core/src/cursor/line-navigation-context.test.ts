...
import { createPosition, selectionContextOf } from "../state";
  ...
  if (opts.footerParas !== undefined) {
    templateBlocks.push(
      ...
      buildBlock({
        ...
        inlineContent: inlineContent([text(opts.footerParas[0])]),
      }),
    );
  }
...
describe("#327 — line-navigation context isolation (positioned tree)", () => {
  it("ArrowUp from the body's first line does NOT enter the header (positioned path)", () => {
    ...
    expect(hdr[0].absoluteY).toBeLessThan(body0[0].absoluteY);
