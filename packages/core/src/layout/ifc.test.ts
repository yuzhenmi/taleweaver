...
describe("IFC — trailing-whitespace hang at soft wrap (#311, pre-wrap fidelity)", () => {
  // Helper: collect a line's text-run leaves (recursing into inline boxes) in
  // visual order, returning { x, text } for content-reach assertions.
  function textRunLeaves(line: import("./layout-box-v2").LineBox) {
  ...
  function linesOf(text: string, width: number, ws: ComputedStyle["whiteSpace"]) {
    const tree = cascadePass(
      createElementBox("p", { display: "block", whiteSpace: ws }, [
        createTextBox("t", {}, text),
      ]),
    ...
    if (out.type !== "block") throw new Error("?");
    const lines = out.children.filter((c) => c.type === "line");
    return lines.filter((l): l is import("./layout-box-v2").LineBox => l.type === "line");
  }

  it("DISCRIMINATING: 'ab cd   ' @ 40px under pre-wrap is 1 line (3 trailing spaces hang); 'cd' is reached", () => {
    // 8px/char. unit1="ab "(24, word16/trail8), unit2="cd"+3sp (40, word16/trail24).
    // After pushing unit1 (cW=24), test unit2: 24 + wordWidth(16) = 40 <= 40 ⇒ NO wrap.
    // OLD (pre-change) test was 24 + totalWidth(40) = 64 > 40 ⇒ 2 lines. (See no-regression
    // capture comment below; the new behavior is 1 line.)
    const lines = linesOf("ab cd   ", 40, "pre-wrap");
    expect(lines).toHaveLength(1);
    // The single line reaches "cd".
    const leaves = textRunLeaves(lines[0]);
    const joined = leaves.map((l) => l.text).join("");
    expect(joined).toContain("cd");
  });

  it("NO over-hang: 'aaaa   bb' @ 56px under pre-wrap is 2 lines (interior spaces count)", () => {
    // unit1="aaaa"+3sp (56, word32/trail24), unit2="bb"(16, word16/trail0).
    // After pushing unit1 (cW=56, full — pushUnit unchanged), test unit2:
    // 56 + wordWidth(16) = 72 > 56 ⇒ WRAP. The 3 spaces are now INTERIOR
    // between "aaaa" and "bb", so they MUST count. Guards the rejected
    // over-correction (subtracting prior trailing from currentWidth).
    const lines = linesOf("aaaa   bb", 56, "pre-wrap");
    expect(lines).toHaveLength(2);
  });

  it("trailing spaces at paragraph end hang: 'abc        ' (8 trailing) @ 40px is 1 line owning all offsets", () => {
    const text = "abc        "; // "abc" + 8 spaces = 11 chars
    const lines = linesOf(text, 40, "pre-wrap");
    expect(lines).toHaveLength(1);
    // The single line owns the full state range (no wrap stranded any chars).
    expect(lines[0].inlineOffsetStart).toBe(0);
    expect(lines[0].inlineOffsetEnd).toBe(text.length);
  });

  it("NO-REGRESSION (normal): 'ab cd   ' under white-space:normal wraps exactly as today", () => {
    // Under normal, the hang gate is OFF, so the overflow test still uses
    // unit.totalWidth (byte-identical to pre-change). tokenize("ab cd   ",
    // "normal") emits one token per trailing-space char → ["ab"," ","cd"," ",
    // " "," "]; the grouper makes unit1="ab "(24, word16/trail8) and
    // unit2="cd   "(40, word16/trail24). cW=24 after unit1, then
    // 24 + totalWidth(40) = 64 > 40 ⇒ WRAP. So the CURRENT (pre-change) normal
    // behavior is 2 lines. The gate keeps normal byte-identical (still
    // totalWidth), so this stays 2. (Captured against pre-change code: 2 lines.)
    // A gating bug that accidentally hung normal would use wordWidth(16):
    // 24 + 16 = 40 ≤ 40 ⇒ 1 line, failing here.
    const lines = linesOf("ab cd   ", 40, "normal");
    const all = lines.map((l) => textRunLeaves(l).map((leaf) => leaf.text).join("")).join("|");
    expect(all).toContain("ab");
    expect(all).toContain("cd");
    expect(lines.length).toBe(2);
  });

  it("offset continuity across a wrap: nextLine.inlineOffsetStart === prevLine.inlineOffsetEnd (pre-wrap)", () => {
    // A two-line pre-wrap case where the hang does NOT collapse the wrap:
    // "aaaa   bb" @ 56px wraps (interior spaces count). Offsets must stay
    // contiguous: the 3 interior spaces are owned by line 1 (its trailing run),
    // and line 2 starts exactly where line 1 ended.
    const lines = linesOf("aaaa   bb", 56, "pre-wrap");
    expect(lines.length).toBe(2);
    expect(lines[1].inlineOffsetStart).toBe(lines[0].inlineOffsetEnd);
    // The whole document's offsets are covered.
    expect(lines[0].inlineOffsetStart).toBe(0);
    expect(lines[lines.length - 1].inlineOffsetEnd).toBe("aaaa   bb".length);
  });

  it("I1: a standalone all-space wrap unit has wordWidth 0 (interior space-run never forces a wrap)", () => {
    // A leading/orphan space-run under pre-wrap is emitted as an all-space
    // WrapUnit; its wordWidth (totalWidth − trailingWhitespaceWidth) is 0, so
    // the overflow test always accepts it onto the current line. We assert this
    // at the unit level: a 3-space leading run has width 24 but wordWidth 0.
    // (Use collectTokens + the public grouper is internal, so we assert via the
    // layout outcome: "   abc" under pre-wrap @ 40px keeps the 3 leading spaces
    // and "abc" on one line — the all-space run did not force a premature wrap.)
    const lines = linesOf("   abc", 48, "pre-wrap");
    // 3 leading spaces (24px, wordWidth 0) + "abc" (24px) = 48px content fits.
    expect(lines).toHaveLength(1);
    expect(lines[0].inlineOffsetStart).toBe(0);
    expect(lines[0].inlineOffsetEnd).toBe(6);
  });
});
