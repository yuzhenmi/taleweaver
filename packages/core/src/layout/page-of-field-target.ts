...
/**
 * Injected parent-lookup capability (built in the editor layer from `State`),
 * so the layout layer resolves a nested block's ancestor chain WITHOUT importing
 * `State` (preserving the render→cascade→layout State-decoupling). Returns the
 * block's parent id, or `null` at the root / for an unknown id.
 */
