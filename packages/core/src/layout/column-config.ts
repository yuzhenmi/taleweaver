 ...
 *
 * Per-section multi-column configuration (Google Docs Format ▸ Columns).
 *
 ...
 * the doc default; `section-column-config.resolveColumnConfig` validates a
 * section's overrides, and `section-plan` threads the resolved config onto each
 * `SectionBoundary` (mirroring the per-section `PageConfig` machinery, C.2b-2).
 *
 * INERT in slice 1: this is vocabulary + plan-threading only — NO layout pass
 * consumes `columnConfig` yet, so a doc carrying column attrs still lays out
 * single-column. The `MultiColumnBox` + column fragmentation land in slice 2.
 */
import type { BorderStyle, Color } from "../styles";
