...
describe("fitColumnsOnPage", () => {
  it("columnCount 1 reduces to a single fitOnePage (single-column pages unaffected)", () => {
    ...
    expect(cols.columns[0].childrenCount).toBe(single.childrenCount);
    expect(cols.columns[0].resumeOut).toEqual(single.resumeOut);
  ...
  it("distributes contiguous doc-order runs into columns in order, overflowing the last", () => {
    ...
    expect(r.columns[1].resumeInto).toEqual(r.columns[0].resumeOut);
    ...
    expect(r.pageResumeOut?.resumeChildToken).toEqual(r.columns[1].resumeOut);
  });
  ...
  it("a single paragraph fragments across the column boundary (head in col0, tail in col1)", () => {
    ...
    expect(r.columns[1].resumeOut).toBeNull();
  ...
  it("resumeInto seeds column 0 (a page continuing a prior page's overflow)", () => {
    ...
    expect(page2.columns[0].startIndex).toBe(2);
    expect(page2.columns[0].childrenCount).toBe(1); // block 2
    expect(page2.columns[1].childrenCount).toBe(1); // block 3
