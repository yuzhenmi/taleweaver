...
export default function moveToLineBelow(): Command {
  return (cursorExtension: CursorExtension): Transformation => {
    ...
    if (!nextLineBox) {
      transformation.addOperation(new MoveTo(offset - lineBoxLevelPosition.getSelectableOffset() + lineBox.getSelectableSize() - 1));
    } else {
      ...
      transformation.addOperation(new MoveTo(offset - lineBoxLevelPosition.getSelectableOffset() + lineBox.getSelectableSize() + targetLineSelectableOffset));
    }
    return transformation;
  };
}
