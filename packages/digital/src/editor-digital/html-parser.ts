 ...
 * wraps each DOM `Node` as an `HtmlNode`. Pass `browserHtmlParser` into the
 * `EditorConfig` (`htmlParser` field) so HTML paste is decoded by core.
 *
 * Intentionally mirrors `packages/print/src/html-parser.ts` in shape — both are
 * pure DOM glue with no layout dependency. They are kept separate to respect the
 * one-way dependency chain (digital does NOT depend on print).
 *
