...
function hasStyleAtNode(
  ...
  for (let i = 0; i < path.length; i++) {
    if (node.type === "span") {
      if (property === "fontWeight" && node.properties.fontWeight === "bold") return true;
      if (property === "fontStyle" && node.properties.fontStyle === "italic") return true;
      if (property === "textDecoration" && node.properties.textDecoration === "underline") return true;
    }
...
export function getFormatState(editorState: EditorState): FormatState {
  ...
  if (isCollapsed(selection)) {
    ...
    // Expanded: use isFullyStyled
    bold = isFullyStyled(state, selection, "fontWeight");
    italic = isFullyStyled(state, selection, "fontStyle");
    underline = isFullyStyled(state, selection, "textDecoration");
  }
