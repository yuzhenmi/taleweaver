...
function tocEntryCenter(
  pageBox: core.LayoutBox,
  target: core.BlockId,
): { x: number; y: number } | null {
  let found: { x: number; y: number } | null = null;
  const walk = (box: core.LayoutBox, parentX: number, parentY: number): void => {
...
function getPage0(editor: core.EditorState): core.LayoutBox {
...
describe("TOC click-to-navigate gesture", () => {
  ...
  function lastSetSelection(dispatch: ReturnType<typeof vi.fn>): core.Span | null {
    for (let i = dispatch.mock.calls.length - 1; i >= 0; i--) {
      const a = dispatch.mock.calls[i][0] as core.EditorAction;
  ...
  it("a plain click on a TOC entry jumps the caret to its heading", () => {
    const { editor, h2Id } = seededTocEditor();
    const container = mount();
    const dispatch = vi.fn();
    const controller = createEditorController(container, {
      measurer, dispatch, pageHeight: 800, pageGap: 24,
    });
    ...
    const center = tocEntryCenter(getPage0(editor), h2Id);
