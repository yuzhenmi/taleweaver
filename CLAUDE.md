...
- **Parallel implementer agents:** never dispatch multiple implementer (file-writing) subagents that share the main checkout. Observed failures: commit scope-leak (Agent B's untracked files get included in Agent A's `git add` / commit), commit-undone-by-reset race (Agent B does `git reset HEAD~1` for its own cleanup and accidentally undoes A's commit), and in-place file revert race (Agent B's file snapshot was taken before A's edit, B's write overwrites). Either **serialize implementer dispatch** OR pass `isolation: "worktree"` so each agent gets its own checkout. Reviewer agents (read-only) can run in parallel freely. After a worktree agent commits, the controller merges back from the main checkout — be sure to `cd /Users/hansyu/code/taleweaver/` (not the worktree directory) before `git merge`.
